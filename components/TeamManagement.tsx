		...
		if (editingTeam) {
			...
			const team = teams.find(
				(t) =>
					...
					t.leader &&
					t.leader._id === employeeId,
			);
			return team;
		} else {
			// When creating, check all teams
			const team = teams.find(
				(t) => t.leader && t.leader._id === employeeId,
			);
			return team;
		}
	};
		...
		<div className='space-y-4'>
			...
			{teamsLoading && teams.length === 0 ? (
				<div className='bg-white rounded-lg shadow-sm border border-gray-100 p-12 text-center'>
					<LoadingDots />
					<p className='text-gray-600 font-secondary mt-4'>
						Loading teams...
					</p>
				</div>
							...
							<motion.div
								...
								<div className='flex items-start justify-between mb-3'>
									<div className='flex-1'>
										...
										{team.description && (
											<p className='text-sm text-gray-600 font-secondary mb-3'>
												{team.description}
											</p>
										)}
									</div>
								...
								<div className='space-y-2'>
									<div className='flex items-center gap-2 text-sm'>
										...
										<span className='font-medium text-gray-700 font-secondary'>
											Leader:
										</span>
										<div className='flex items-center gap-2'>
											<UserAvatar
								...
								{team.members.length > 0 && (
									<div className='mt-3 pt-3 border-t border-gray-200'>
										<p className='text-xs font-medium text-gray-600 mb-2 font-secondary'>
											Team Members:
										</p>
							...
							<form onSubmit={handleSubmit} className='space-y-5'>
											...
											{leaderDropdownOpen && (
												<motion.div
													...
													<div className='max-h-60 overflow-y-auto'>
														{filteredLeaderEmployees.length ===
														0 ? (
															...
															filteredLeaderEmployees.map(
																(emp) => {
																	const teamAsLeader =
																		getEmployeeTeamAsLeader(
																			emp._id,
																		);
																	const teamAsMember =
																		getEmployeeTeamAsMember(
																			emp._id,
																		);
																	const isAvailable =
																		isEmployeeAvailable(
								...
								<div>
									...
									<div
										...
										<div className='min-h-[48px] w-full px-3 py-2 text-sm text-gray-700 border-2 border-gray-200 rounded-lg focus-within:ring-2 focus-within:ring-primary/50 focus-within:border-primary/50 outline-none font-secondary bg-white/80 backdrop-blur-sm shadow-sm transition-all flex flex-wrap gap-2 items-center'>
											...
											<input
												...
												placeholder={
													selectedMemberEmployees.length ===
													0
										...
										<AnimatePresence>
											{memberDropdownOpen &&
												memberSearchTerm &&
												filteredMemberEmployees.length >
													0 && (
													<motion.div
														initial={{
															opacity: 0,
									...
									<p className='text-xs text-gray-500 mt-2 font-secondary'>
										Note: The team leader is automatically
					...
					deleteModal.team ? (
						<div className='space-y-1'>
							...
							<div>
								...
								{deleteModal.team.leader?.name || "No Leader"}
							</div>
							<div>
								<span className='font-semibold'>Members:</span>{" "}
								{deleteModal.team.members.length}
							</div>
						</div>
					) : null
				}
