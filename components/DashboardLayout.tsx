...
export default function DashboardLayout({
	...
	const menu =
		role === "admin" ? adminMenu : role === "hr" ? hrMenu : employeeMenu;
	...
	return (
		<div className='min-h-screen bg-gray-50'>
			...
			{/* Sidebar */}
			<aside
				className={`fixed top-0 left-0 h-full w-56 bg-primary shadow-lg z-50 transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${
				...
				<div className='flex flex-col h-full'>
					<div className='p-4'>
						...
						<p className='text-xs text-gray-100 capitalize font-secondary mt-1'>
							{role} Portal
						</p>
					</div>
							...
							.map((item) => {
								...
								return (
									<Link
										key={item.name}
										href={item.href}
										className={`flex items-center gap-2.5 px-3 py-2 transition-colors text-sm rounded-xl ${
											isActive
							...
							{role === "hr" && (
								<>
									<Link
										...
										className={`flex items-center mt-1 gap-2.5 px-3 py-2 rounded-lg transition-colors text-sm ${
											mounted &&
					...
					<div className='p-3'>
						<div className='px-3 py-2 mb-2 flex items-center gap-2 bg-[#000077] rounded-lg'>
							...
							<div className='flex-1 min-w-0'>
								<p className='text-xs font-medium text-gray-100 font-primary truncate'>
									{userName}
								</p>
			...
			<div className='lg:pl-56'>
				...
				{/* Page content */}
				<main className='bg-[#eef3ff] p-3 pb-16 lg:pb-4'>
					{children}
				</main>
			</div>
		</div>
