...
export default function ResignationManagement({
	...
	const [exitProcessModal, setExitProcessModal] = useState<{
		isOpen: boolean;
		resignation: Resignation | null;
		step: string;
		stepTitle: string;
	...
	const processResignation = (r: any) => {
		...
		const noticePeriodStartDate = r.noticePeriodStartDate
			? typeof r.noticePeriodStartDate === "string"
				? r.noticePeriodStartDate
				: new Date(r.noticePeriodStartDate).toISOString()
			: undefined;
		const noticePeriodEndDate = r.noticePeriodEndDate
			? typeof r.noticePeriodEndDate === "string"
				? r.noticePeriodEndDate
				: new Date(r.noticePeriodEndDate).toISOString()
			: undefined;
		return {
			...r,
			assets: assetsArray,
	...
	const fetchResignations = async () => {
		try {
			...
			if (res.ok) {
				setResignations(
					(data.resignations || []).map(processResignation),
				);
			} else {
	...
	const handleApproveConfirm = async () => {
		...
		try {
			...
			const res = await fetch(
				`/api/resignation/${approveModal.resignationId}`,
				{
					method: "PUT",
	...
	const handleDeleteConfirm = async () => {
		...
		try {
			...
			const res = await fetch(
				`/api/resignation/${deleteModal.resignationId}`,
				{
					method: "DELETE",
				},
			);
		...
		try {
			...
			switch (step) {
				...
				case "fnf":
					...
					if (data.date) requestBody.date = data.date;
					if (data.amount)
						requestBody.amount = parseFloat(data.amount);
					if (data.notes) requestBody.notes = data.notes;
					break;
				case "documents":
			...
			const res = await fetch(
				`/api/resignation/${exitProcessModal.resignation._id}/process`,
				{
					method: "PATCH",
					headers: { "Content-Type": "application/json" },
			...
			if (res.ok) {
				...
				toast.error(
					responseData.error || "Failed to update exit process",
				);
			}
		} catch (err: any) {
			toast.error(err.message || "An error occurred");
	...
	const filteredResignations = useMemo(() => {
		return resignations.filter((resignation) => {
			...
			const matchesStatus =
				filterStatus === "all" || resignation.status === filterStatus;
	...
	const paginatedResignations = useMemo(() => {
		...
		return filteredResignations.slice(
			startIndex,
	...
	const rejectedCount = resignations.filter(
		(r) => r.status === "rejected",
	).length;
		...
		<div className='space-y-3'>
			...
			<div className='grid grid-cols-1 md:grid-cols-3 gap-3'>
				<div className='bg-yellow-50/80 backdrop-blur-sm rounded-xl shadow-lg border border-yellow-200/50 p-3'>
					<div className='flex items-center justify-between'>
						<div>
							...
							<p className='text-xl font-bold text-yellow-700 font-primary'>
								{pendingCount}
							</p>
						</div>
				...
				<div className='bg-green-50/80 backdrop-blur-sm rounded-xl shadow-lg border border-green-200/50 p-3'>
					<div className='flex items-center justify-between'>
						<div>
							<p className='text-xs text-green-700 font-secondary'>
								Approved
							</p>
							<p className='text-xl font-bold text-green-700 font-primary'>
								{approvedCount}
							</p>
						</div>
				...
				<div className='bg-red-50/80 backdrop-blur-sm rounded-xl shadow-lg border border-red-200/50 p-3'>
					<div className='flex items-center justify-between'>
						<div>
							<p className='text-xs text-red-700 font-secondary'>
								Rejected
							</p>
							<p className='text-xl font-bold text-red-700 font-primary'>
								{rejectedCount}
							</p>
						</div>
							...
							<motion.div
								...
								<div className='space-y-2 text-xs flex-1'>
									...
									<div className='flex items-center gap-2'>
										...
										<span
											className='text-gray-600 font-secondary truncate'
											title={resignation.userId.email}>
											{resignation.userId.email}
										</span>
									</div>
											...
											<div className='flex-1'>
												...
												<div className='space-y-0.5'>
													...
													{(() => {
														...
														if (
															resignation.noticePeriodComplied
														) {
															return (
																<p className='text-xs'>
									...
									<div className='pt-1 border-t border-gray-100'>
										...
										{(() => {
											...
											return (
												<p className='text-xs text-gray-400 ml-5 italic'>
													No assets assigned
												</p>
											);
										})()}
										...
										<div className='flex items-center gap-2 mb-1.5'>
											...
											{(() => {
												...
												const hasNotes =
													...
													resignation.handoverNotes.trim() !==
														"";

												if (
													resignation.knowledgeTransferCompleted
												) {
													return (
									...
									{resignation.feedback && (
										<div className='pt-1 border-t border-gray-100'>
											...
											<p
												className='text-xs text-gray-600 ml-5 line-clamp-2'
												title={resignation.feedback}>
												{resignation.feedback}
											</p>
										</div>
									)}
									...
									{resignation.status === "rejected" &&
										resignation.rejectionReason && (
											<div className='pt-1 border-t border-red-100'>
												...
												<p
													className='text-xs text-red-600 ml-5 line-clamp-2'
													title={
														resignation.rejectionReason
													}>
													{
										...
										<div className='grid grid-cols-2 gap-1.5'>
											{(() => {
												...
												return (
													<button
														...
														Notice Period
														{isCompleted && (
															<CheckCircle className='w-3.5 h-3.5 text-green-600 absolute -top-1 -right-1' />
														)}
													</button>
												);
											})()}
											...
											{(() => {
												...
												const allApproved = [
													...
													(dept) =>
														clearances[
															dept as keyof typeof clearances
														]?.status ===
														"approved",
												);
												return (
											...
											{(() => {
												const isCompleted =
													resignation.systemAccessDeactivated ===
													true;
												return (
													<button
														type='button'
