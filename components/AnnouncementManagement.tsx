...
export default function AnnouncementManagement() {
	...
	const [showPollResponses, setShowPollResponses] =
		useState<Announcement | null>(null);
	...
	const handleDelete = async () => {
		...
		try {
			const res = await fetch(
				`/api/announcements/${deleteModal.announcement._id}`,
				{
					method: "DELETE",
				},
			);
									...
									<div className='flex items-center gap-3'>
										...
										<div>
											...
											<p className='text-sm text-gray-600 font-secondary'>
												{showPollResponses.title}
											</p>
										</div>
									</div>
									<button
										onClick={() =>
											setShowPollResponses(null)
										}
										className='p-2 hover:bg-gray-100 rounded-lg transition-colors'>
										<X className='w-5 h-5 text-gray-500' />
									</button>
								...
								<div className='p-6'>
									<div className='mb-6'>
										...
										<p className='text-sm text-gray-500'>
											Total Votes:{" "}
											{showPollResponses.poll.options.reduce(
												(sum, opt) =>
									...
									<div className='space-y-4'>
										{showPollResponses.poll.options.map(
											(option, index) => {
												const voteCount =
													option.votes.length;
												const totalVotes =
												...
												return (
													<div
														...
														<div className='flex items-center justify-between mb-3'>
															<span className='font-secondary font-semibold text-gray-800'>
																{option.text}
															</span>
