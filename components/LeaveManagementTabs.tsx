...
import { Plus, X, Calendar, Trash2, Users } from "lucide-react";
...
export default function LeaveManagementTabs({
	...
	const [activeTab, setActiveTab] = useState<"requests" | "types" | "allot">(
		"requests",
	);
	...
	const toggleLeaveType = (leaveType: LeaveType) => {
		const isSelected = selectedLeaveTypes.some(
			(lt) => lt.leaveTypeId === leaveType._id,
		);
		if (isSelected) {
			setSelectedLeaveTypes((prev) =>
				prev.filter((lt) => lt.leaveTypeId !== leaveType._id),
			);
		} else {
	...
	const updateLeaveTypeDays = (leaveTypeId: string, days: string) => {
		setSelectedLeaveTypes((prev) =>
			prev.map((lt) =>
				lt.leaveTypeId === leaveTypeId ? { ...lt, days } : lt,
			),
		);
	};
	...
	const updateLeaveTypeHours = (leaveTypeId: string, hours: string) => {
		setSelectedLeaveTypes((prev) =>
			prev.map((lt) =>
				lt.leaveTypeId === leaveTypeId ? { ...lt, hours } : lt,
			),
		);
	};
	...
	const updateLeaveTypeMinutes = (leaveTypeId: string, minutes: string) => {
		setSelectedLeaveTypes((prev) =>
			prev.map((lt) =>
				lt.leaveTypeId === leaveTypeId ? { ...lt, minutes } : lt,
			),
		);
	};
	...
	const allFilteredEmployeesSelected = useMemo(() => {
		return (
			...
			filteredEmployeeIds.every((id) => selectedEmployees.includes(id))
		);
	...
	const handleEditEmployee = (employeeId: string, employeeLeaves: any[]) => {
		...
		if (role === "hr") {
			...
			if (employeeId === currentUserId) {
				toast.error(
					"HR users cannot edit their own allotted leaves. Please contact an Admin.",
				);
				return;
			}
		}
	...
	const handleAllotLeave = async (e: React.FormEvent) => {
		...
		if (invalidLeaveTypes.length > 0) {
			toast.error(
				"Please enter valid values for all selected leave types (days for regular leaves, hours/minutes for shortday leaves)",
			);
			return;
		}
		...
		try {
			...
			if (editingEmployeeId && editingEmployeeLeaves.length > 0) {
				...
				if (failedDeletes.length > 0) {
					toast.error(
						`Failed to delete ${failedDeletes.length} old leave(s). Please try again.`,
					);
					setLoading(false);
					return;
				}
				...
				deletedLeaveIds = editingEmployeeLeaves.map(
					(leave) => leave._id,
				);
			}
			...
			for (const userId of selectedEmployees) {
				for (const lt of selectedLeaveTypes) {
					...
					if (isShortDayLeaveType) {
						...
						const minutesStr = lt.minutes?.trim() || "";
						const hoursValue =
							hoursStr !== "" ? parseInt(hoursStr) : 0;
						const minutesValue =
							minutesStr !== "" ? parseInt(minutesStr) : 0;
						...
						if (isNaN(hoursValue) && isNaN(minutesValue)) {
							console.error(
								"Invalid shortday leave: both hours and minutes are NaN",
								lt,
							);
						...
						if (hoursValue === 0 && minutesValue === 0) {
							console.error(
								"Invalid shortday leave: both hours and minutes are 0",
								lt,
							);
							continue; // Skip this allocation
						}
			...
			if (data.errorCount > 0) {
				toast.error(
					`Allotted ${data.successCount} leave(s), but ${data.errorCount} failed. Check console for details.`,
				);
				console.error("Allotment errors:", data.errors);
			} else {
				toast.success(
					editingEmployeeId
						? `Successfully updated ${data.successCount} leave(s)`
						: `Successfully allotted ${data.successCount} leave(s)`,
				);
			}
	...
	const leaveRequests = useMemo(() => {
		...
		const filtered = leaves.filter((leave) => {
			...
			if (
				leave.reason &&
				/penalty|late.*clock.*in|exceeded.*max.*late/i.test(
					leave.reason,
				)
			) {
				return false;
			...
			if (role === "hr" && session?.user) {
				const userId =
					typeof leave.userId === "object" && leave.userId?._id
								...
								<div
									...
									<div className='flex items-start justify-between'>
										<div className='flex-1'>
											...
											{type.description && (
												<p className='text-sm text-gray-600 font-secondary mb-2'>
													{type.description}
												</p>
											)}
											{type.maxDays && (
										...
										<button
											onClick={() =>
												handleDeleteLeaveTypeClick(type)
											}
										...
										<div className='absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto'>
											...
											<div className='px-2 py-2 border-b border-gray-200'>
												<label className='flex items-center gap-2 p-2 hover:bg-gray-50 rounded cursor-pointer'>
													<input
														ref={
															selectAllEmployeesRef
														}
														type='checkbox'
														checked={
															allFilteredEmployeesSelected
														}
														onChange={
															toggleSelectAllFilteredEmployees
														}
												...
												{filteredLeaveTypes.map(
													(lt) => (
														<label
															...
															<input
																type='checkbox'
																checked={selectedLeaveTypes.some(
																	(slt) =>
																		slt.leaveTypeId ===
																		lt._id,
																)}
							...
							{selectedLeaveTypes.length > 0 && (
								<div className='space-y-3'>
									<label className='block text-sm font-medium text-gray-700 font-secondary'>
										Enter Values for Each Leave Type{" "}
										<span className='text-red-500'>*</span>
									</label>
									<div className='space-y-2 max-h-60 overflow-y-auto'>
										{selectedLeaveTypes.map((lt) => {
											const leaveTypeName =
												lt.leaveTypeName?.toLowerCase() ||
												"";
											const isShortDayLeaveType =
