...
export default function ResignationForm({
	...
	const handleNext = () => {
		if (currentStep < steps.length) {
			...
			if (currentStep === 2) {
				if (
					!formData.noticePeriodStartDate ||
					!formData.noticePeriodEndDate
				) {
			...
			if (currentStep === 5) {
				if (!formData.clearancesAcknowledged) {
					toast.error(
						"Please acknowledge the Department Clearances requirement",
					);
					return;
				}
			}
				...
				<div className='bg-gradient-to-r from-primary to-purple-600 text-white p-3 flex items-center justify-between'>
					<div>
						...
						<p className='text-xs text-white/90 mt-0.5'>
							Complete all steps to submit your resignation
						</p>
					</div>
					<button
						onClick={onClose}
							...
							<motion.div
								...
								<div className='bg-blue-50 border border-blue-200 rounded-lg p-2.5 mb-2'>
									...
									<p className='text-[10px] text-blue-800'>
										Please provide your notice period start
										and end dates. The notice period must
								...
								<div>
									...
									/>
									<p className='text-[10px] text-gray-500 mt-0.5'>
										This should match your last working day
										(Resignation Date)
									</p>
								</div>
							</motion.div>
							...
							<motion.div
								...
								<div className='bg-green-50 border border-green-200 rounded-lg p-2.5 mb-2'>
									...
									<p className='text-[10px] text-green-800'>
										Please provide details about knowledge
										transfer and handover of your
										responsibilities.
									</p>
								</div>
...
- Any other relevant information for smooth transition...'
									/>
									<p className='text-[10px] text-gray-500 mt-0.5'>
										Detailed handover notes help ensure a
										smooth transition
									</p>
								</div>
							</motion.div>
							...
							<motion.div
								...
								<div className='bg-orange-50 border border-orange-200 rounded-lg p-2.5 mb-2'>
									...
									<p className='text-[10px] text-orange-800'>
										Select all company assets currently in
										your possession. These assets must be
										returned before your last working day.
									</p>
								</div>
								...
								<div className='grid grid-cols-2 md:grid-cols-3 gap-2'>
									{companyAssets.map((asset) => {
										...
										const isSelected =
											formData.assets.includes(asset.id);
										return (
											<label
												key={asset.id}
												...
												<Icon
													className={`w-4 h-4 flex-shrink-0 ${isSelected ? "text-primary" : "text-gray-500"}`}
												/>
												<span
													className={`text-xs font-medium ${isSelected ? "text-primary" : "text-gray-700"}`}>
													{asset.label}
												</span>
											</label>
										);
							...
							<motion.div
								...
								<div className='bg-blue-50 border border-blue-200 rounded-lg p-2.5 mb-2'>
									...
									<p className='text-[10px] text-blue-800'>
										...
										is mandatory. This includes reporting
										manager, IT, Admin/Facilities, and
										Finance. Exit clearance status will be
										updated by each department directly in
										the HRIS portal.
									</p>
								</div>
									...
									<div className='bg-yellow-50 border-2 border-yellow-200 rounded-lg p-3'>
										<label className='flex items-start gap-2 cursor-pointer'>
											<input
												type='checkbox'
												checked={
													formData.clearancesAcknowledged
												}
												onChange={(e) =>
													setFormData({
														...formData,
