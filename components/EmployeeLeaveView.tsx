...
export default function EmployeeLeaveView({
	...
	const handleRequestLeave = async (e: React.FormEvent) => {
		...
		const isHalfDay =
			selectedLeaveType?.name?.toLowerCase().includes("halfday") ||
			selectedLeaveType?.name?.toLowerCase().includes("half-day") ||
			selectedLeaveType?.name?.toLowerCase().includes("half day");
		const isShortDay =
			selectedLeaveType?.name?.toLowerCase().includes("shortday") ||
			selectedLeaveType?.name?.toLowerCase().includes("short-day") ||
			selectedLeaveType?.name?.toLowerCase().includes("short day");
		const isMedicalLeave = selectedLeaveType?.name
			?.toLowerCase()
			.includes("medical");
		...
		if (
			isShortDay &&
			formData.shortDayFromTime &&
			formData.shortDayToTime
		) {
			const fromTime = new Date(
				`2000-01-01T${formData.shortDayFromTime}`,
			);
		...
		if (formData.leaveType && formData.startDate && formData.endDate) {
			...
			const allottedLeavesList = leaves.filter(
				(leave) => leave.allottedBy,
			);
			...
			if (allottedLeave) {
				...
				const leaveTypeName =
					typeof allottedLeave.leaveType === "object"
						? allottedLeave.leaveType?.name?.toLowerCase() || ""
						: "";
				const isShortDayLeaveType =
					leaveTypeName.includes("shortday") ||
				...
				if (
					...
					const toTime = new Date(
						`2000-01-01T${formData.shortDayToTime}`,
					);
					const diffMs = toTime.getTime() - fromTime.getTime();
					const requestedTotalMinutes = Math.floor(
						diffMs / (1000 * 60),
					);
					const requestedHours = Math.floor(
						requestedTotalMinutes / 60,
					);
					const requestedMinutes = requestedTotalMinutes % 60;

					const totalRemainingMinutes =
						((allottedLeave as any).remainingHours || 0) * 60 +
						((allottedLeave as any).remainingMinutes || 0);
					...
					if (totalRemainingMinutes < requestedTotalMinutes) {
						...
						toast.error(
							`Insufficient leave balance. You have ${formatHoursMinutes(remainingHours, remainingMinutes)} remaining, but requested ${formatHoursMinutes(requestedHours, requestedMinutes)}.`,
						);
						setLoading(false);
						return;
					}
					...
					const remainingDays =
						allottedLeave.remainingDays !== undefined
							? allottedLeave.remainingDays
							: allottedLeave.days || 0;
					if (remainingDays < requestedDays) {
						const requestedDaysDisplay =
							requestedDays === 0.5
								? "0.5"
								: requestedDays < 1 && requestedDays > 0
									? requestedDays.toFixed(2)
									: requestedDays.toString();
						toast.error(
							`Insufficient leave balance. You have ${remainingDays} days remaining, but requested ${requestedDaysDisplay} ${requestedDays === 0.5 || (requestedDays < 1 && requestedDays > 0) ? "day" : "days"}.`,
						);
						setLoading(false);
						return;
					}
	...
	return (
		<div className='space-y-6'>
			...
			<div className='flex items-center justify-between'>
				<div>
					<h2 className='text-lg font-primary font-semibold text-gray-800'>
						My Allotted Leaves
					</h2>
			...
			{allottedLeaves.length === 0 && loading ? (
				...
				<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'>
					{allottedLeaves.map((leave) => {
						...
						const leaveTypeName =
							typeof leave.leaveType === "object"
								? leave.leaveType?.name?.toLowerCase() || ""
								: "";
						const isShortDayLeaveType =
							leaveTypeName.includes("shortday") ||
							leaveTypeName.includes("short-day") ||
						...
						if (isShortDayLeaveType) {
							...
							remainingHours =
								leave.remainingHours !== undefined
									? leave.remainingHours
									: totalHours;
							remainingMinutes =
								leave.remainingMinutes !== undefined
									? leave.remainingMinutes
									: totalMinutes;
							...
							const totalUsedMinutes =
								totalAllottedMinutes - totalRemainingMinutes;
				...
				<div className='fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto'>
					<motion.div
						...
						<form
								...
								{leaveTypes.length === 0 ? (
									<div className='p-3 bg-gray-50/80 backdrop-blur-sm rounded-lg border border-gray-200/50'>
										<p className='text-xs text-gray-600 font-secondary text-center'>
											You need to have leaves allotted by
											admin/HR before you can request
											leave
										</p>
									</div>
								) : (
											...
											return (
												<motion.button
													...
													<div className='flex items-center justify-center gap-1.5'>
														<span
															className={`text-[12px] font-medium font-primary ${isSelected ? "text-primary" : "text-gray-700"}`}>
															{type.name}
														</span>
													</div>
												</motion.button>
													...
													formData.shortDayToTime && (
														<div className='mt-2 p-2 bg-blue-100/80 rounded-lg border border-blue-200/50'>
															<div className='space-y-1'>
																<div className='flex items-center justify-between'>
																	<span className='text-[10px] font-medium text-gray-700 font-secondary'>
																		Time
																		Range:
																	</span>
							...
							{formData.leaveType &&
								(() => {
									const selectedAllottedLeave = leaves.find(
										(leave) => {
											const leaveTypeId =
												typeof leave.leaveType ===
												"object"
									...
									if (selectedAllottedLeave) {
										// Check if this is a shortday leave type
										const leaveTypeName =
											typeof selectedAllottedLeave.leaveType ===
											"object"
										...
										if (isShortDayLeaveType) {
											...
											const consumedHours = Math.floor(
												totalConsumedMinutes / 60,
											);
											const consumedMins =
												totalConsumedMinutes % 60;
											...
											return (
												<motion.div
													...
													<h3 className='text-xs font-semibold text-gray-800 mb-2 font-primary'>
														Leave Balance
													</h3>
													<div className='grid grid-cols-3 gap-3'>
														<div className='text-center'>
															...
															<div className='text-[10px] text-gray-600 mt-0.5 font-secondary'>
																Total
															</div>
														</div>
														<div className='text-center'>
								...
								(() => {
									...
									if (
										...
										const remainingAfterRequest =
											totalRemainingMinutes -
											requestedTotalMinutes;
										const remainingHoursAfter = Math.floor(
											remainingAfterRequest / 60,
										);
										const remainingMinutesAfter =
											remainingAfterRequest % 60;
										...
										return (
											<motion.div
												...
												<div className='flex items-center justify-between'>
													<div>
														<div className='text-xs font-medium text-gray-700 font-secondary'>
															Requested Time
														</div>
														<div
															className={`text-xl font-bold mt-0.5 font-primary ${
																isInsufficient
																	? "text-red-600"
																	: "text-green-600"
															}`}>
								...
								<motion.div
									...
									{loadingTeamMembers ? (
										...
										<p className='text-xs text-gray-600 font-secondary'>
											No team members have leave during
											this period
										</p>
									) : (
										<div className='space-y-2'>
					...
					deleteModal.leave ? (
						<div className='space-y-1'>
							...
							<div>
								...
								{format(
									new Date(deleteModal.leave.startDate),
									"MMM dd, yyyy",
								)}{" "}
								-{" "}
								{format(
									new Date(deleteModal.leave.endDate),
									"MMM dd, yyyy",
								)}
							</div>
							<div>
								<span className='font-semibold'>Days:</span>{" "}
								{deleteModal.leave.days === 0.5
									? "0.5 day"
									: deleteModal.leave.days &&
										  deleteModal.leave.days < 1 &&
										  deleteModal.leave.days > 0
				...
				<div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
					<div className='bg-white/80 backdrop-blur-md rounded-lg p-6 max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto'>
						...
						<div className='space-y-4'>
							<div>
								<h4 className='text-sm font-semibold text-gray-700 mb-2 font-secondary'>
									Reason:
								</h4>
								<p className='bg-white/70 p-3 rounded-md text-gray-700 text-sm whitespace-pre-wrap break-words'>
									{reasonModal.reason}
								</p>
							</div>
							{reasonModal.medicalReport && (
								<div>
									<div className='flex items-center justify-between mb-2'>
										<h4 className='text-sm font-semibold text-gray-700 font-secondary'>
											Medical Report:
										</h4>
										<button
											onClick={() =>
												setMedicalReportMinimized(
									...
									{!medicalReportMinimized && (
										<div className='bg-red-50 p-3 rounded-md border border-red-200'>
											{reasonModal.medicalReport.startsWith(
												"data:image/",
											) ? (
												...
												<div className='space-y-2'>
													<img
														...
														onError={(e) => {
															console.error(
																"Error loading medical report image:",
																e,
															);
															(
