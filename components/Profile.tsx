		...
		try {
			...
			if (res.ok) {
				...
				if (data.user.dateOfBirth) {
					try {
						...
						if (!isNaN(dobDate.getTime())) {
							...
							const day = String(dobDate.getDate()).padStart(
								2,
								"0",
							);
				...
				setFormData({
					...
					joiningYear: data.user.joiningYear
						? String(data.user.joiningYear)
						: "",
					currentPassword: "",
					newPassword: "",
					confirmPassword: "",
		...
		try {
			...
			if (uploadRes.ok && uploadData.url) {
				...
				if (updateRes.ok) {
					...
					toast.error(
						updateData.error || "Failed to update profile picture",
					);
			...
			const updateData: any = {
				...
				dateOfBirth:
					formData.dateOfBirth && formData.dateOfBirth.trim() !== ""
						...
						: null,
				joiningYear:
					formData.joiningYear && formData.joiningYear.trim() !== ""
							...
							<div className='relative group'>
								...
								{uploading && (
									<motion.div
										...
										<div className='text-white text-xs font-secondary'>
											Uploading...
										</div>
									</motion.div>
								)}
