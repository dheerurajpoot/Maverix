...
import PayslipGenerationModal from "@/components/PayslipGenerationModal";
...
export default function HRDashboard() {
	...
	const statCards = [
		...
		},
		{
			...
			percentage:
				totalEmployees > 0
					? Math.round((stats.clockedInToday / totalEmployees) * 100)
					: 0,
			trend: "up" as "up" | "down",
		...
		},
		{
			...
			percentage:
				totalEmployees > 0
					? Math.round((stats.pendingLeaves / totalEmployees) * 100)
					: 0,
		...
		},
		{
			...
			percentage:
				totalEmployees > 0
					? Math.round((stats.onLeaveToday / totalEmployees) * 100)
					: 0,
			trend: "down" as "up" | "down",
		...
		},
		{
			...
			percentage:
				totalEmployees > 0
					? Math.round((stats.weeklyOffToday / totalEmployees) * 100)
					: 0,
			trend: "down" as "up" | "down",
					...
					<div className='flex items-center justify-between gap-4 flex-wrap bg-primary-500 p-3 rounded-xl shadow-md'>
						<div className='flex items-center gap-2'>
							...
							<div>
								...
								<p className='text-sm text-gray-400 mt-0.5 font-secondary'>
									Welcome back,{" "}
							...
							return (
								<motion.div
									...
									<div className='flex items-start justify-between mb-4'>
										<div
											className={`${stat.iconBg} p-3 rounded-full shadow-sm`}>
											<Icon className='w-6 h-6 text-white' />
										</div>
									</div>
									<div>
										<p className='text-sm text-gray-700 font-secondary mb-2 font-semibold'>
											{stat.title}
										</p>
										{loading ? (
											...
											<>
												<p className='text-3xl font-primary font-bold text-gray-900 mb-2'>
													{stat.value}
												</p>
												<div className='flex items-center gap-1'>
													<span
														className={`text-xs font-semibold font-secondary ${stat.trend === "up" ? "text-green-600" : "text-red-600"}`}>
														{stat.trend === "up"
															? "↑"
															: "↓"}{" "}
														{stat.percentage}%
