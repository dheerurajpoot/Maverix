...
export default function EmployeeFinancePage() {
	...
	const [docPreview, setDocPreview] = useState<{
		[key: string]: { pan?: string; aadhar?: string };
	}>({});
	...
	const fetchUserDocuments = async () => {
		try {
			...
			if (res.ok) {
				...
				const panCard =
					data.panCardImage &&
					typeof data.panCardImage === "string" &&
					data.panCardImage.trim() !== "" &&
					data.panCardImage !== "null"
						? data.panCardImage
						: undefined;
				const aadharCard =
					data.aadharCardImage &&
					typeof data.aadharCardImage === "string" &&
					data.aadharCardImage.trim() !== "" &&
					data.aadharCardImage !== "null"
						? data.aadharCardImage
						: undefined;
				...
				const updatedUser = {
					...
					location:
						data.user.location !== undefined
							...
							: null,
					panNumber:
						data.user.panNumber !== undefined
							? data.user.panNumber
							: null,
					aadharNumber:
						data.user.aadharNumber !== undefined
	...
	const handleDocumentUpload = async (
		...
		try {
			...
			if (compressedSizeKB > 100) {
				console.warn(
					`Warning: Compressed size (${compressedSizeKB.toFixed(2)}KB) exceeds target of 100KB`,
				);
			}
			...
			const updateRes = await fetch("/api/profile/documents", {
				...
				body: JSON.stringify({
					[documentType === "pan"
						? "panCardImage"
						: "aadharCardImage"]: uploadData.url,
				}),
			});
			...
			if (updateData.panCardImage || updateData.aadharCardImage) {
				setUserDocuments((prev) => ({
					...
					aadharCardImage:
						updateData.aadharCardImage || prev.aadharCardImage,
				}));
			} else {
			...
			toast.success(
				`${documentType === "pan" ? "PAN" : "Aadhar"} card uploaded successfully`,
			);
	...
	const handleRemoveDocument = async (documentType: "pan" | "aadhar") => {
		try {
			...
			toast.success(
				`${documentType === "pan" ? "PAN" : "Aadhar"} card removed successfully`,
			);

			// Update state immediately
			setUserDocuments((prev) => ({
				...prev,
				[documentType === "pan" ? "panCardImage" : "aadharCardImage"]:
					undefined,
			}));
						...
						<motion.div
							...
							<div className='flex items-center justify-between'>
								<div>
									...
									<p className='text-3xl font-primary font-bold text-gray-800'>
										INR
									</p>
								</div>
					...
					<div className='bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-white/50 p-5'>
							...
							<div className='flex items-center gap-3'>
								...
								<div>
									...
									<p className='text-xs text-gray-600 font-secondary'>
										Manage your bank account and personal
										details
									</p>
								</div>
							</div>
						...
						{showBankForm ? (
							...
							<div className='mt-3 space-y-3'>
								<div className='grid grid-cols-1 md:grid-cols-3 gap-3'>
									<div className='p-3 bg-gray-50/50 rounded-lg border border-gray-100'>
										...
										<p className='text-sm font-semibold text-gray-900 font-secondary'>
											{user.bankName || "N/A"}
										</p>
									</div>
									<div className='p-3 bg-gray-50/50 rounded-lg border border-gray-100'>
							...
							<div className='mt-3 p-3 bg-yellow-50/50 rounded-lg border border-yellow-200/50'>
								<p className='text-xs text-gray-600 font-secondary'>
									No bank details added yet. Click &quot;Add
									Bank Details&quot; to add your bank
									information.
								</p>
							</div>
						)}
						...
						<div className='flex items-center justify-between mb-4'>
							<div className='flex items-center gap-3'>
								...
								<div>
									...
									<p className='text-xs text-gray-600 font-secondary'>
										Upload your PAN and Aadhar card
										documents
									</p>
								</div>
							</div>
							<button
								...
								title={
									documentsExpanded ? "Minimize" : "Expand"
								}>
								{documentsExpanded ? (
						...
						{documentsExpanded && (
							<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
								...
								<div className='space-y-2'>
									<label className='text-xs font-semibold text-gray-700 font-secondary'>
										PAN Card
									</label>
									{(userDocuments.panCardImage &&
										userDocuments.panCardImage.trim() !==
											"") ||
									(docPreview["general"]?.pan &&
										docPreview["general"]?.pan.trim() !==
											"") ? (
								...
								<div className='space-y-2'>
									...
									{(userDocuments.aadharCardImage &&
										userDocuments.aadharCardImage.trim() !==
											"") ||
									(docPreview["general"]?.aadhar &&
										docPreview["general"]?.aadhar.trim() !==
											"") ? (
										<div className='relative'>
											<img
												src={
												...
												onError={(e) => {
													console.error(
														"Error loading Aadhar card image",
													);
													// If image fails to load, clear it from state
													setUserDocuments(
														(prev) => ({
					...
					{loading ? (
						...
						<div className='bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-white/50 p-8 text-center'>
							...
							<p className='text-xs text-gray-500 font-secondary mt-1'>
								Your salary information will appear here once
								allocated
							</p>
						</div>
					) : (
						...
						<motion.div
							...
							<div className='flex items-center justify-between mb-5'>
								<div className='flex items-center gap-3'>
									...
									<div>
										...
										<p className='text-xs text-gray-600 font-secondary'>
											Enter your password to view salary
											details
										</p>
									</div>
								</div>
							...
							<form
								...
								<div>
									...
									{passwordError && (
										<p className='mt-1.5 text-xs text-red-600 font-secondary'>
											{passwordError}
										</p>
									)}
								</div>
