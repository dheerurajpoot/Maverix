...
export default function HRLeaveAllotmentPage() {
	...
	const fetchAllottedLeaves = async () => {
		try {
			...
			if (res.ok && data.leaves) {
				...
				const allotted = data.leaves.filter(
					(leave: any) => leave.allottedBy,
				);
	...
	const someFilteredEmployeesSelected = filteredEmployeeIds.some((id) =>
			...
			<div className='space-y-4'>
				<div className='flex items-center justify-between'>
					<div>
						...
						<p className='text-sm text-gray-600 mt-0.5 font-secondary'>
							Allot leaves to employees in bulk
						</p>
					</div>
					<button
								...
								{selectedLeaveTypes.length > 0 && (
									<div className='space-y-3'>
										<label className='block text-sm font-medium text-gray-700 font-secondary'>
											Enter Days for Each Leave Type{" "}
				...
				<div className='bg-white rounded-lg shadow-sm border border-gray-100'>
					<div className='p-4 border-b border-gray-200'>
						...
						<p className='text-sm text-gray-600 mt-0.5 font-secondary'>
							View and manage all allotted leaves
						</p>
					</div>
					<div className='p-4'>
						{loadingLeaves ? (
							...
							<AllottedLeavesList
								...
								onEditEmployee={(
									...
									const currentUserId = (session?.user as any)
										?.id;
