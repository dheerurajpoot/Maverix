...
export default function AttendanceManagement({
	...
	const [userClockInTime, setUserClockInTime] = useState<string | undefined>(
		undefined,
	);
	const [selectedPenalty, setSelectedPenalty] = useState<any | null>(null);
	const [userWeeklyOff, setUserWeeklyOff] = useState<string[]>([]);
	const [leavesForMonth, setLeavesForMonth] = useState<Set<string>>(
		new Set(),
	);
	...
	const [statusFilter, setStatusFilter] = useState<
		"all" | "weekOff" | "leave" | "present" | "late"
	>(
		initialFilter === "leave" ||
			initialFilter === "weekOff" ||
			...
			try {
				...
				const penaltyPromises = Array.from(attendanceDates).map(
					async (dateStr) => {
						try {
							const res = await fetch(
								`/api/attendance/penalty?date=${dateStr}`,
							);
							const data = await res.json();
							...
							console.error(
								`Error fetching penalty for date ${dateStr}:`,
								err,
							);
						}
					},
		...
		if (!isAdminOrHR) {
			...
			try {
				...
				if (res.ok && data.user) {
					...
					setUserWeeklyOff(
						Array.isArray(data.user.weeklyOff)
							? data.user.weeklyOff
							: [],
					);
				}
				...
				if (res.ok && data.leaves) {
					...
					data.leaves.forEach((leave: any) => {
						if (leave.status === "approved" && !leave.allottedBy) {
							...
							const isPenaltyLeave =
								leave.reason &&
								/penalty|clock.*in.*late|exceeded.*max.*late|auto.*deduct/i.test(
									leave.reason,
								);
							if (!isPenaltyLeave) {
								...
								if (
									...
									const rangeStart =
										leaveStart > monthStart
											...
											: monthStart;
									const rangeEnd =
										leaveEnd < endDate ? leaveEnd : endDate;
	...
	const isLateClockIn = useCallback(
		(clockInTime: string, userClockInTime?: string): boolean => {
	...
	const attendanceByDateMap = useMemo(() => {
		...
		initialAttendance.forEach((att) => {
			...
			if (
				!existing ||
				new Date(att.clockIn) > new Date(existing.clockIn)
			) {
				map.set(dateStr, att);
			}
		});
			...
			employees: employees
				.filter((emp) => {
					...
					if (statusFilter !== "all") {
						...
						const isWeeklyOff =
							emp.weeklyOff?.includes(selectedDayName) || false;
						const hasAttendance = !!attendanceMap.get(emp._id);
						const isPresent =
							hasAttendance && !isOnLeave && !isWeeklyOff;
						const isLate =
							hasAttendance &&
							!isOnLeave &&
							!isWeeklyOff &&
						...
						if (statusFilter === "weekOff" && !isWeeklyOff)
							return false;
						if (statusFilter === "leave" && !isOnLeave)
							return false;
							...
							<div className='flex items-center gap-3'>
								...
								<div className='flex items-center gap-2'>
									...
									<div>
										...
										{isToday && (
											<p className='text-xs text-blue-600 font-secondary'>
												Today
											</p>
										)}
									</div>
								...
								<button
									...
									disabled={isToday}>
									<ChevronRight
										className={`w-5 h-5 ${isToday ? "text-gray-300" : "text-gray-600"}`}
									/>
								</button>
							</div>
											...
											selectedPenalty.lateArrivals
												...
												<div className='mt-2 pt-2 border-t border-gray-200'>
													...
													<div className='space-y-1.5 max-h-48 overflow-y-auto'>
														{selectedPenalty.lateArrivals.map(
															(
																arrival: any,
																idx: number,
															) => (
		...
		return (
			<>
				<div className='space-y-4'>
						...
						<div className='flex items-center justify-between gap-4'>
							<div className='flex items-center gap-2'>
								...
								<button
									...
									<ChevronRight
										className={`w-5 h-5 ${isCurrentMonthCheck ? "text-gray-300" : "text-gray-600"}`}
									/>
								</button>
							</div>
					...
					{/* Attendance Table */}
					{!allDaysWithAttendance ||
					allDaysWithAttendance.length === 0 ? (
						<div className='bg-white/95 backdrop-blur-xl rounded-md shadow-lg border border-white/50 p-12 text-center'>
							<Clock className='w-12 h-12 text-gray-400 mx-auto mb-3' />
							<p className='text-gray-600 font-secondary'>
								No days found for {monthDisplay}
							</p>
						</div>
					) : (
						<div className='bg-white/95 backdrop-blur-xl rounded-md shadow-lg border border-white/50'>
													...
													<tr
														...
														<td className='px-4 py-3 whitespace-nowrap'>
															{hasClockedIn ? (
																...
																<span
																	className={`text-xs font-secondary italic ${
																		isWeeklyOff
																			? "text-purple-500"
																			: isOnLeave
																				? "text-orange-500"
