	...
	const getStepStatus = (stepId: string) => {
		...
		switch (stepId) {
			...
			case "fnf":
				...
				if (resignation.fnfStatus === "processing")
					return "in-progress";
				// If resignation is approved, FnF is in-progress
				if (isApproved) return "in-progress";
				return "pending";
			case "documents":
				if (resignation.exitDocuments?.experienceLetter)
					return "completed";
					...
					<div className='space-y-2 text-sm'>
						...
						{resignation.approvedAt && (
							<p>
								...
								{format(
									new Date(resignation.approvedAt),
									"MMM dd, yyyy",
								)}
							</p>
						)}
					...
					<div className='space-y-2 text-sm'>
						{resignation.noticePeriodStartDate && (
							<p>
								...
								{format(
									new Date(resignation.noticePeriodStartDate),
									"MMM dd, yyyy",
								)}
							</p>
						)}
						{resignation.noticePeriodEndDate && (
							<p>
								...
								{format(
									new Date(resignation.noticePeriodEndDate),
									"MMM dd, yyyy",
								)}
							</p>
						)}
						<p>
							<strong>Complied:</strong>{" "}
							{resignation.noticePeriodComplied ? "Yes" : "No"}
						</p>
					</div>
				);
					...
					<div className='space-y-2 text-sm'>
						...
						{resignation.handoverCompletedDate && (
							<p>
								...
								{format(
									new Date(resignation.handoverCompletedDate),
									"MMM dd, yyyy",
								)}
							</p>
						)}
					...
					<div className='space-y-2 text-sm'>
						...
						{resignation.assetsReturnDate && (
							<p>
								...
								{format(
									new Date(resignation.assetsReturnDate),
									"MMM dd, yyyy",
								)}
							</p>
						)}
				...
				return (
					<div className='space-y-3 text-sm'>
						{departments.map((dept) => {
							const clearance = clearances[dept.key] || {
								status: "pending",
							};
							return (
								<div
					...
					<div className='space-y-2 text-sm'>
						{resignation.exitInterviewDate && (
							<p>
								...
								{format(
									new Date(resignation.exitInterviewDate),
									"MMM dd, yyyy",
								)}
							</p>
						)}
			...
			case "fnf":
				return (
					<div className='space-y-2 text-sm'>
						<p>
							<strong>Status:</strong>{" "}
							{resignation.fnfStatus || "pending"}
						</p>
						{resignation.fnfAmount && (
							<p>
								<strong>Amount:</strong> ₹
								{resignation.fnfAmount.toLocaleString()}
							</p>
						)}
						{resignation.fnfProcessedDate && (
							<p>
								...
								{format(
									new Date(resignation.fnfProcessedDate),
									"MMM dd, yyyy",
								)}
							</p>
						)}
					...
					<div className='space-y-2 text-sm'>
						...
						{!docs.experienceLetter && !docs.relievingLetter && (
							<p className='text-gray-500'>
								Documents will be available after FnF completion
							</p>
						)}
					</div>
				);
					...
					<div className='space-y-2 text-sm'>
						{resignation.exitClosedDate && (
							<p>
								...
								{format(
									new Date(resignation.exitClosedDate),
									"MMM dd, yyyy",
								)}
							</p>
						)}
						{resignation.exitClosedBy && (
							<p>
								...
								{resignation.exitClosedBy.name}
							</p>
						)}
						<p>
							<strong>Status:</strong>{" "}
							{resignation.exitClosed ? "Closed" : "Open"}
						</p>
					</div>
				);
