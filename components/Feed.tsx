...
import { Send, Trash2, MessageSquare, Loader2, X, Eye } from "lucide-react";
...
export default function Feed() {
			...
			setSelectedMentionIndex((prev) => {
				...
				setTimeout(() => {
					const selectedElement =
						mentionDropdownRef.current?.querySelector(
	...
	const parseContent = (
		...
		while ((match = mentionRegex.exec(content)) !== null) {
			...
			if (!mentionedUser) {
				mentionedUser = mentions.find((user) => {
												...
												{filteredUsers.map(
													(user, index) => (
														<button
															...
															{user.designation && (
																<div
																	className={`text-xs px-2 py-0.5 rounded-full font-secondary ${
																		user.role ===
																		"admin"
										...
										<div className='flex-1 min-w-0'>
											<div className='flex items-center gap-1.5 mb-0.5 flex-wrap'>
												...
												{post.userId.designation && (
													<span
														className={`text-[10px] px-1.5 py-0.5 rounded-full font-secondary font-medium ${
															post.userId.role ===
															"admin"
					...
					<motion.div
						...
						<motion.div
							...
							<div className='flex items-center justify-between p-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-purple-50'>
								<h3 className='text-lg font-primary font-bold text-gray-900'>
									Post Details
								</h3>
								<button
									onClick={() => {
									...
									<div className='flex-1'>
										<div className='flex items-center gap-2 mb-1 flex-wrap'>
											...
											{viewingPost.userId.designation && (
												<span
													className={`text-xs px-3 py-1 rounded-full font-secondary font-medium ${
														viewingPost.userId
															.role === "admin"
										...
										<p className='text-sm text-gray-500 font-secondary'>
											{formatDistanceToNow(
												new Date(viewingPost.createdAt),
												{
													addSuffix: true,
												},
											)}
