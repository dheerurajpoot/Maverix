...
export default function AdvancedFilterBar({
	...
	const fromFilter = dateRangeFilter
		? filters.find((f) => f.key === `${dateRangeFilter.key}From`)
		: null;
	const toFilter = dateRangeFilter
		? filters.find((f) => f.key === `${dateRangeFilter.key}To`)
		: null;
	...
	const getFilterLabel = (filter: (typeof filters)[0]) => {
		...
		if (filter.type === "date") {
			return filter.value
				? format(new Date(filter.value), "dd MMM yyyy")
				: filter.label;
		}
							...
							<div className='flex gap-2'>
								...
								<div className='relative' ref={presetRef}>
									<button
										...
										<span>
											{getCurrentPreset || "Today"}
										</span>
										<ChevronDown
											className={`w-4 h-4 text-gray-400 transition-transform duration-200 ${datePresetOpen ? "rotate-180" : ""}`}
										/>
									</button>
									<AnimatePresence>
										{datePresetOpen && (
								...
								<button
									...
									<ChevronDown
										className={`w-4 h-4 text-gray-400 transition-transform duration-200 ${dateRangeOpen ? "rotate-180" : ""}`}
									/>
								</button>
							</div>
