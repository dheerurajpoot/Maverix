...
export default function TimeTrackingWidget() {
				...
				try {
					...
					if (
						...
						if (clockOutRes.ok) {
							...
							setAutoClockOutScheduled(false);
							toast.success(
								"Automatically clocked out at 11:11 PM",
							);
				...
				if (delayMs > 0) {
					setTimeout(async () => {
						try {
							...
							console.error(
								"Error during automatic clock-out:",
								err,
							);
							setAutoClockOutScheduled(false);
						}
	...
	}, [clockedIn, autoClockOutScheduled]);
	...
	const handleClockInOut = async () => {
		try {
			...
			const res = await fetch("/api/attendance", {
				...
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					action: clockedIn ? "clockOut" : "clockIn",
				}),
			});
	...
	const cleanDescription = (description: string): string => {
		...
		const observancePattern =
			/Observance\s+To\s+hide\s+observances,?\s+go\s+to\s+Google\s+Calendar\s+Settings\s+>\s+Holidays\s+in\s+India/gi;
						...
						<motion.div
							...
							<div className='p-3 sm:p-4 md:p-6 flex-1 flex flex-col'>
												...
												{(() => {
													...
													return (
														<>
															...
															<span className='text-2xl sm:text-3xl md:text-4xl font-primary font-bold text-gray-900'>
																:
															</span>
															<span className='text-5xl sm:text-3xl md:text-4xl font-primary font-bold text-primary bg-primary-100 px-2 py-1 sm:px-3 sm:py-2 rounded-md shadow-sm'>
																{minutes}
													...
													(() => {
														...
														return (
															<>
																...
																<span className='text-4xl sm:text-3xl md:text-4xl font-primary font-bold text-gray-900'>
																	:
																</span>
																<span className='text-4xl sm:text-3xl md:text-4xl font-primary font-bold text-primary bg-primary-100 px-2 py-1 sm:px-3 sm:py-2 rounded-md shadow-sm'>
																	{minutes}
									...
									<div className='text-center mb-3 sm:mb-4'>
										<p className='text-xs sm:text-sm font-semibold text-gray-500'>
											...
											<span className='font-bold text-gray-900'>
												{formatClockInDate(clockInTime)}{" "}
												at {formatTimeOnly(clockInTime)}
											</span>
										</p>
									</div>
								)}
								...
								<motion.button
									...
									className={`w-full py-4 sm:py-3 md:py-4 rounded-md font-semibold text-sm sm:text-base transition-all shadow-lg flex items-center justify-center gap-2 mb-3 sm:mb-4 ${
										clockedIn
								...
								<div className='border-t border-gray-200 pt-2 sm:pt-3'>
									<div className='flex items-center gap-1.5 sm:gap-2 mb-2'>
										...
										<h3 className='text-xs sm:text-sm font-primary font-semibold text-gray-900'>
											Recent Attendance
										</h3>
									</div>
										...
										{attendanceLoading ? (
											<div className='text-center py-2 sm:py-3'>
												<LoadingDots
													...
													className='mb-1'
												/>
												<p className='text-[10px] sm:text-xs text-gray-500 font-secondary'>
													Loading...
												</p>
											</div>
												...
												.map((record, index) => (
													<motion.div
														...
														<div className='flex items-center gap-2 sm:gap-2.5 flex-1 min-w-0'>
															<div
																className={`p-1 sm:p-1.5 rounded flex-shrink-0 ${record.clockOut ? "bg-green-100" : "bg-yellow-100"}`}>
																<CheckCircle
																	className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${record.clockOut ? "text-green-600" : "text-yellow-600"}`}
																/>
															</div>
					...
					<div className='p-6 flex-1 flex flex-col'>
						...
						{clockedIn && clockInTime && (
							<div className='text-center mb-6'>
								<p className='text-sm font-semibold text-gray-500'>
									Clocked in:{" "}
									<span className='font-bold text-gray-900'>
										{formatClockInDate(clockInTime)} at{" "}
										{formatTimeOnly(clockInTime)}
									</span>
								</p>
							</div>
						)}
						...
						<div className='border-t border-gray-200 pt-4'>
							<div className='flex items-center gap-2 mb-3'>
								...
								<h3 className='text-sm font-primary font-semibold text-gray-900'>
									Recent Attendance
								</h3>
							</div>
										...
										.map((record, index) => (
											<motion.div
												...
												<div className='flex items-center gap-3 flex-1'>
													<div
														className={`p-1.5 rounded ${record.clockOut ? "bg-green-100" : "bg-yellow-100"}`}>
														<CheckCircle
															className={`w-3.5 h-3.5 ${record.clockOut ? "text-green-600" : "text-yellow-600"}`}
														/>
													</div>
						...
						<div className='flex-1 min-w-0 flex flex-col'>
							...
							<div className='grid grid-cols-7 gap-0.5 flex-1'>
								{calendarDays.map((day, idx) => {
									...
									const hasEvent =
										calendarEvents[dayKey] &&
										calendarEvents[dayKey].length > 0;
