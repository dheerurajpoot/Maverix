...
export default function PayslipGenerationModal({
	isOpen,
	onClose,
}: PayslipGenerationModalProps) {
	...
	const generatePayslip = async () => {
		...
		if (!payslipData.finance) {
			toast.error(
				"No finance data available for this employee. Please add salary information first.",
			);
			return;
		}
		...
		try {
			...
			doc.text(
				"Near Brahmakumaris center",
				pageWidth / 2,
				textStartY + 14,
				{ align: "center" },
			);
			doc.text("Sujatganj, Kanpur -", pageWidth / 2, textStartY + 20, {
				align: "center",
			});
			...
			doc.setFont("helvetica", "bold");
			doc.text(
				`Payslip for the month of ${monthNames[selectedMonth - 1]} ${selectedYear}`,
				pageWidth / 2,
				textStartY + 35,
				{ align: "center" },
			);
			...
			doc.text(
				`Date of joining: ${payslipData.employee.dateOfJoining}`,
				leftColX,
				yPos + 6,
			);
			doc.text(
				`Department: ${payslipData.employee.designation || "N/A"}`,
				leftColX,
				yPos + 12,
			);
			doc.text(
				`Location: ${payslipData.employee.location || "N/A"}`,
				leftColX,
				yPos + 18,
			);
			doc.text(
				`Effective work days: ${payslipData.workDays.effective}`,
				leftColX,
				yPos + 24,
			);
			doc.text(
				`Days in month: ${payslipData.workDays.total}`,
				leftColX,
				yPos + 30,
			);
			...
			doc.text(
				`Bank Name: ${payslipData.employee.bankName || "N/A"}`,
				rightColX,
				yPos,
			);
			doc.text(
				`Bank Account No: ${payslipData.employee.accountNumber || "N/A"}`,
				rightColX,
				yPos + 6,
			);
			...
			doc.text(
				`PAN No: ${payslipData.employee.panNumber || "N/A"}`,
				rightColX,
				yPos + 30,
			);
			...
			if (payslipData.finance) {
				...
				const lopDeduction =
					payslipData.workDays.lop > 0
						? (basicSalary / payslipData.workDays.total) *
							payslipData.workDays.lop
						: 0;
						...
						<motion.div
							...
							<div className='sticky top-0 bg-white border-b border-gray-200 px-5 py-3 flex items-center justify-between z-10'>
								<div className='flex items-center gap-2'>
									...
									<h2 className='text-lg font-semibold text-gray-900 font-primary'>
										Generate Payslip
									</h2>
								</div>
								<button
							...
							<div className='p-5 space-y-4'>
											...
											<div className='max-h-[400px] overflow-y-auto'>
												...
												<div className='bg-gray-50 border-b border-gray-200 px-3.5 py-2.5'>
													<h3 className='text-sm font-semibold text-gray-900 font-primary'>
														Mavericks and Musers
														Media Pvt. Ltd.
													</h3>
													<p className='text-xs text-gray-600 font-secondary mt-0.5'>
														79A, B Block Shyam
														Nagar, Near
														...
														return (
															<div className='space-y-2'>
																<div className='border-b border-gray-200 pb-1.5'>
																	<h4 className='text-xs font-semibold text-gray-900 font-primary mb-1.5'>
																		Earnings
																	</h4>
													...
													<div className='space-y-1.5'>
														...
														<div className='grid grid-cols-2 gap-2 text-xs font-secondary'>
															...
															<span className='font-medium text-red-600 text-right'>
																₹
																{payslipData.finance.deductions.toFixed(
																	2,
																)}
															</span>
														</div>
								...
								{payslipData && (
									<button
										...
										disabled={
											generating || !payslipData.finance
										}
										className='w-full bg-gray-900 hover:bg-gray-800 text-white font-medium py-2.5 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm font-secondary'>
										{generating ? (
											<>
